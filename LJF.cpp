...
int main()
{
    ...
    for (int i = 1; i < n; i++)
    {
        int var, key, larger;
        /*
            "larger" variable will compare the burst time of arrived process having larger value.
            "key" vaiable will have the index of the process that is going to be under execution.
            "var" variable is used to determine the processes that have arrived during execution of previous process.
        */
        larger = p[i].bt;
        var = p[i - 1].ct;
        for (int j = i; j < n; j++)
        {
            if (var >= p[j].at && larger <= p[j].bt)
            {
                larger = p[j].bt;
                key = j;
            }
    ...
    cout << endl
         << endl;
    cout.width(42);
    cout << "ACCORDING TO LJF ALGORITHM" << endl;
